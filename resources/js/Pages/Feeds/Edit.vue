...
const form = useForm({
    ...
    protein: props.feed.protein,
    fat: props.feed.fat,
    fiber: props.feed.fiber,
    tdn: props.feed.tdn,
...
const deleteFeed = () => {
    if (confirm(__('Are you sure you want to delete this feed ingredient?'))) {
        form.delete(route('feeds.destroy', props.feed.id));
    }
};
</script>

<template>
    <Head :title="__('Edit Feed Ingredient')" />
        ...
        <template #header>
            <div class="flex justify-between items-center w-full">
                <div class="flex items-center space-x-3">
                    ...
                    <span class="text-xl font-bold">{{ __('Edit Feed Ingredient') }}</span>
                </div>
                <button @click="deleteFeed" class="text-red-500 hover:text-red-700 transition h-10 w-10 flex items-center justify-center rounded-xl hover:bg-red-50">
        ...
        <div class="max-w-4xl mx-auto py-6">
            <Card class="shadow-sm border-orange-100">
                <form @submit.prevent="submit" class="p-8 space-y-6">
                    ...
                    <div class="flex items-center justify-end pt-6 border-t border-orange-50 space-x-3">
